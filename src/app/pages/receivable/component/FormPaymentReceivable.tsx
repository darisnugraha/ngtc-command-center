...
const mapState = (state: RootState) => {
  if (state.receivable.feedbackDetailOC !== undefined) {
    return {
      initialValues: {
        no_order_confirmation: state.receivable.feedbackDetailOC[0].no_order_konfirmasi,
        total_price: state.receivable.feedbackDetailOC[0].total_harga,
        down_payment: state.receivable.feedbackDetailOC[0].total_harga / 2,
        remaining_payment: state.receivable.feedbackDetailOC[0].sisa_bayar,
      },
    };
  }
};

const FormPaymentReceivable: React.FC<InjectedFormProps<{}, Props>> = (props: any) => {
  ...
  const getValue = async (event: { target: { files: any[] } }) => {
    ...
    dispatch(change('FormPaymentReceivable', 'foto', base64));
  ...
  const Bank = useSelector<RootState>(({ receivable }) => receivable.listBank);
...
const form = reduxForm<{}, Props>({
  ...
  form: 'FormPaymentReceivable',
  touchOnChange: true,
  validate: PaymentValidation,
})(FormPaymentReceivable);
